...
class ParallelProductionScraper:
    ...
    async def get_organic_traffic_via_api_OLD(self, domain: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            # Domaine nettoyé automatiquement
        ...
        try:
            ...
            if result.get('data') and result['data'].get('result'):
                ...
                if overview_data and len(overview_data) > 0:
                    ...
                    # Métriques OverviewTrend récupérées
    ...
    async def get_folder_id_for_domain(self, domain: str) -> Optional[str]:
        ...
        try:
            ...
            # Recherche FID pour domaine
            
            # 1. Récupérer la liste des projets/dossiers existants
            # Appel API folders/selector-list
    ...
    async def scrape_traffic_analysis(self, domain: str, date_range: str, existing_metrics: Dict[str, str] = None):
        ...
        try:
            ...
            # Conversion Rate uniquement (visits supprimé selon la doc)
            ...
            # Récupérer uniquement conversion_rate
            conversion_rate = await scrape_conversion_rate()
    ...
    def validate_metrics_status(self, metrics_data: dict) -> str:
        """
        Valide le statut des métriques de manière adaptative.
        
        Args:
            metrics_data: Dictionnaire contenant les métriques
            
        Returns:
            'completed' si toutes les métriques sont valides, 'partial' sinon
        """
        # Liste des métriques attendues (adaptative) - correspond aux clés de format_analytics_for_api()
        expected_metrics = [
            'organic_traffic',
            'paid_search_traffic', 
            'bounce_rate',
        ...
        for metric in expected_metrics:
            ...
            if self._is_metric_valid(value):
                valid_metrics_count += 1
        
        # Logique adaptative : completed si toutes les métriques sont valides
        if valid_metrics_count == total_metrics:
            return 'completed'
        else:
            return 'partial'

    def _is_metric_valid(self, value: str) -> bool:
        """
        Vérifie si une métrique est valide.
        ...
        """
        if not value or value == '' or value == 'na' or value == 'Sélecteur non trouvé':
            return False
        return True
            ...
            for i, shop in enumerate(shops, 1):
                try:
                    ...
                    if result == 'na':
                        ...
                        analytics_data['scraping_status'] = 'na'
                        ...
                        # Enregistrer en BDD
                        analytics_data = self.format_analytics_for_api()
                        # Validation adaptative des métriques
                        status = self.validate_metrics_status(analytics_data)
                        # Passer le statut calculé à l'API
                        analytics_data['scraping_status'] = status
                        api.update_shop_analytics(shop_id, analytics_data)

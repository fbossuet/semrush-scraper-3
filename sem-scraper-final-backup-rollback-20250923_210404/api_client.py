...
class APIClient:
    ...
    def __init__(self):
        ...
        """
        self.base_url = f"{url_manager.get_base_url()}/dpa/rpc"
        self.engagement_base_url = f"{url_manager.get_base_url()}/analytics/ta/targ/v2/engagement"
    ...
    async def call_rpc_api(self, page, method: str, params: dict, worker_id: int = 0):
        ...
        # Vérifier et renouveler la session Noxtools si nécessaire
        if url_manager.get_auth_method() == "noxtools":
            await auth_manager.ensure_noxtools_session(page, worker_id)
        
        # Navigation vers la page analytics selon la méthode d'authentification
        if url_manager.should_navigate_to_analytics():
            target_url = url_manager.get_analytics_url()
            try:
                await page.goto(target_url, wait_until='domcontentloaded', timeout=15000)
                await asyncio.sleep(2)
            except Exception as e:
                logger.warning(f"⚠️ Worker {worker_id}: Navigation vers {target_url} échouée: {e}")
                logger.info(f"🔄 Worker {worker_id}: Tentative de navigation vers {url_manager.get_base_url()}...")
                try:
                    await page.goto(url_manager.get_base_url(), wait_until='domcontentloaded', timeout=15000)
                    await asyncio.sleep(2)
                except Exception as e2:
                    logger.warning(f"⚠️ Worker {worker_id}: Navigation vers {url_manager.get_base_url()} échouée: {e2}")
                    logger.info(f"🔄 Worker {worker_id}: Continuation sans navigation...")
        else:
            logger.info(f"🌐 Worker {worker_id}: Pas de navigation nécessaire (méthode: {url_manager.get_auth_method()})")
        ...
        """, {
            ...
            'authMethod': url_manager.get_auth_method()
        })
    ...
    async def call_engagement_api(self, page, domain: str, worker_id: int = 0):
        ...
        try:
            await page.goto(url_manager.get_analytics_url(), wait_until='domcontentloaded', timeout=20000)
            await asyncio.sleep(1)
        except Exception as e:
            logger.warning(f"⚠️ Worker {worker_id}: Préparation même-origine engagement échouée: {e}")

        # Appel same-origin, URL relative, cookies inclus, sans headers additionnels
        api_url_rel = f"/analytics/ta/targ/v2/engagement?target={domain_clean}&device_type=desktop"

        result = await page.evaluate("""
            async (data) => {
                const attempt = async () => {
                    try {
                        // Vérifier l'origine selon la méthode d'authentification
                        const expectedHost = data.authMethod === 'noxtools' ? 'semrush1.semrush.pw' : 'sam.mytoolsplan.xyz';
                        if (!location.host.includes(expectedHost)) {
    ...
    async def call_organic_summary_api(self, page, domain: str, worker_id: int = 0, target_date: Optional[str] = None):
        ...
        # 3. Navigation vers la page analytics selon la méthode d'authentification
        if url_manager.should_navigate_to_analytics():
            target_url = url_manager.get_analytics_url()
            await page.goto(target_url, wait_until='domcontentloaded', timeout=30000)
        else:
            logger.info(f"🌐 Worker {worker_id}: Pas de navigation nécessaire pour engagement (méthode: {url_manager.get_auth_method()})")
        await asyncio.sleep(2)
    ...
    async def call_organic_overview_trend_api(self, page, domain: str, worker_id: int = 0, target_date: Optional[str] = None):
        ...
        if url_manager.should_navigate_to_analytics():
            ...
            logger.info(f"🌐 Worker {worker_id}: Pas de navigation nécessaire pour overview-trend (méthode: {url_manager.get_auth_method()})")

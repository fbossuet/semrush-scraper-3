...
/**
 * Traite les données en lot avec préservation des IDs
 */
  ...
  for (let i = 0; i < shopsData.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (rawShopData) => {
      try {
        ...
        const shopId = await shopRepo.upsertPreserveId(adaptedShopData);
        if (shopId) {
...
async function updateDatabase() {
  logProgress('🔄 Mise à jour de la base de données (PRÉSERVATION DES IDs)...');
  ...
  try {
    ...
    // Traitement optimisé en lot avec préservation des IDs
    logProgress('🔄 Traitement des données en lot (préservation des IDs)...');
    ...
    logProgress('💡 Les IDs sont préservés pour maintenir la cohérence avec analytics');

...
class TrendTrackAPI:
        ...
        try:
            ...
            cursor.execute("""
                SELECT s.id, s.shop_name, s.shop_url, s.scraping_status, s.scraping_last_update, 
                       s.creation_date, s.monthly_visits, s.monthly_revenue, s.live_ads, 
                       s.page_number, s.scraped_at, s.updated_at, s.project_source, s.external_id, s.metadata, s.year_founded
                FROM shops s 
                ORDER BY s.shop_url
            """)
    ...
    def get_shop_by_domain(self, domain: str) -> Optional[Dict]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                # Rechercher par shop_url en construisant l'URL complète
                search_urls = [f"https://{domain}", f"http://{domain}", f"https://www.{domain}", f"http://www.{domain}"]
                for url in search_urls:
                    cursor.execute("SELECT id, shop_name, shop_url FROM shops WHERE shop_url = ?", (url,))
                    row = cursor.fetchone()
                    if row:
                        return {
                            'id': row[0],
                            'shop_name': row[1],
                            'shop_url': row[2],
                            'domain': domain  # Pour compatibilité
                        }
                return None
    ...
    def add_shop(self, shop_data: Dict) -> bool:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    ...
                    (shop_name, shop_url, monthly_visits, monthly_revenue, live_ads, 
                     creation_date, page_number, project_source, updated_at) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    shop_data.get('shop_name', ''),
                    shop_data.get('shop_url', ''),
                    shop_data.get('monthly_visits', ''),
                    shop_data.get('monthly_revenue', ''),
                    shop_data.get('live_ads', ''),
                    shop_data.get('creation_date', ''),
                    shop_data.get('page_number', ''),
                    shop_data.get('project_source', ''),
                    datetime.now(timezone.utc).isoformat()
                ))
                conn.commit()
                logger.info(f"✅ Boutique ajoutée: {shop_data.get('shop_name', 'N/A')}")
                return True
                
        except Exception as e:
            logger.error(f"❌ Erreur ajout boutique {shop_data}: {e}")
            return False
        ...
        try:
            ...
            cursor.execute("""
                SELECT organic_traffic, bounce_rate, avg_visit_duration, branded_traffic, 
                       conversion_rate, visits, traffic, percent_branded_traffic, 
                       paid_search_traffic
                FROM analytics 
                WHERE shop_id = ?
            """, (shop_id,))
    ...
    def get_all_shops_with_analytics(self, limit: int = None) -> List[Dict]:
        ...
        try:
            ...
            shops_with_analytics.sort(key=lambda x: x.get('scraping_status') or '', reverse=True)
    ...
    def update_year_founded(self, shop_id: int, year_founded: str) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                UPDATE shops 
                SET year_founded = ?, updated_at = ?
                WHERE id = ?
            """, (year_founded, datetime.now(timezone.utc).isoformat(), shop_id))
            
            conn.commit()
            logger.info(f"✅ Année de fondation mise à jour pour boutique {shop_id}: {year_founded}")
            return True
            
        except Exception as e:
            logger.error(f"❌ Erreur mise à jour année de fondation {shop_id}: {e}")
            return False
    
    def get_shops_without_year_founded(self, limit: int = None) -> List[Dict]:
        ...
        try:
            ...
            query = """
                SELECT id, shop_name, shop_url, scraping_status
                FROM shops 
                WHERE (year_founded IS NULL OR year_founded = '') 
                AND shop_url IS NOT NULL AND shop_url != ''
                ORDER BY id
            """
            ...
            logger.info(f"📊 {len(shops)} boutiques sans année de fondation")
            return shops
            
        except Exception as e:
            logger.error(f"❌ Erreur récupération boutiques sans année: {e}")
            return []
